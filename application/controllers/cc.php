...
class Cc extends CI_Controller {
	...
	/*
	private function returnUserId($fbId)
    {
	...
	private function getOpponentUserId($fbId){
			$query = $this->db->query("select user_id from cc_user where fb_id='".$fbId."'");
			$result = $query->result();
	...
	public function vscpu($accessToken){
		
		/******************** API Start Module ********************/
		$this->output->set_content_type('application/json');
		$output = array();
		$player_team_batting = array();
		$player_team_bowling = array();
		$opponent_team_batting = array();
		$opponent_team_bowling = array();
		$errCode = 0;
		...
		if($userAuthenticated==1){
			...
			$cpuId =  rand(1, 1);
			//echo $userId;
			$query = $this->db->query("select a.player_id,CONCAT(player_firstname,' ',player_lastname) as player_name,
case when length(player_firstname)>2 
then CONCAT(substr(player_firstname,1,1),' ',player_lastname)
else CONCAT(player_firstname,' ',player_lastname)
end as player_shortname,
player_nationality,player_batting_ranking
from cc_userplayer a
join cc_player b
on a.player_id=b.player_id
join cc_player_attributes c
on b.icc_player_id=c.icc_player_id
where a.user_id=".$userId."
order by player_batting_ranking");
			$result = $query->result();
			foreach($result as $row){
				array_push($player_team_batting,array(
						'id'=>$row->player_id,
						'name'=>$row->player_name,
						'short_name'=>$row->player_shortname,
						'batting_rank'=>$row->player_batting_ranking
					));
			}
...
player_nationality,player_bowling_ranking
from cc_userplayer a
join cc_player b
on a.player_id=b.player_id
join cc_player_attributes c
on b.icc_player_id=c.icc_player_id
where a.user_id=".$userId."
order by player_bowling_ranking");
			$result = $query->result();
			foreach($result as $row){
				array_push($player_team_bowling,array(
						'id'=>$row->player_id,
						'name'=>$row->player_name,
						'short_name'=>$row->player_shortname,
						'batting_rank'=>$row->player_bowling_ranking
					));
			}
...
order by player_batting_ranking");
			$result = $query->result();
			foreach($result as $row){
				array_push($opponent_team_batting,array(
						'id'=>$row->player_id,
						'name'=>$row->player_name,
						'short_name'=>$row->player_shortname,
						'batting_rank'=>$row->player_batting_ranking
...
order by player_bowling_ranking");
			$result = $query->result();
			foreach($result as $row){
				array_push($opponent_team_bowling,array(
						'id'=>$row->player_id,
						'name'=>$row->player_name,
						'short_name'=>$row->player_shortname,
						'batting_rank'=>$row->player_bowling_ranking
			...
			array_push($output,$player_team_batting);
			array_push($output,$player_team_bowling);
			array_push($output,$opponent_team_batting);
			array_push($output,$opponent_team_bowling);
		}
	...
	public function vsplayer($accessToken,$fbId){
		...
		$errCode = 0;
		$userId = $this->getUserId($accessToken);
		$opponentUserId = $this->getOpponentUserId($fbId);
		$userAuthenticated = 0;
...
join cc_player_attributes c
on b.icc_player_id=c.icc_player_id
where a.user_id=".$opponentUserId."
order by player_batting_ranking");
			$result = $query->result();
			foreach($result as $row){
				array_push($opponent_team_batting,array(
